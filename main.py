...
# 페이지 설정
st.set_page_config(page_title="단타 전투 머신 (Pro)", layout="wide")
...
def get_investor_data(date_str):
    ...
    try:
        ...
        df = df.sort_values(by='외국인', ascending=False) # 기본은 외국인 순
        return df
    except:
        return pd.DataFrame()
...
# 탭 구성 (수급 분석 추가)
tab1, tab2, tab3, tab4 = st.tabs(["🏆 스나이퍼", "💰 수급 포착(New)", "🔮 정밀 분석", "📡 AI 스캐너"])
...
with tab1:
    if not all_df.empty:
        ...
        st.dataframe(all_df[['종목명', '종가', '등락률', '신호', '거래대금(억)']].head(20), hide_index=True, use_container_width=True)

# [Tab 2] 수급 포착 (New Feature)
with tab2:
    ...
    if st.button("💰 수급 데이터 불러오기 (클릭)"):
        with st.spinner("KRX에서 데이터를 가져오는 중..."):
            ...
            if not inv_df.empty:
                # 1. 쌍끌이 (교집합)
                # 외인과 기관이 둘 다 10억 이상 산 종목 (금액 단위가 원이라 가정 시 큰 숫자)
                # pykrx 단위는 '원' 입니다. 10억 = 1,000,000,000
                ...
                if not double_buy.empty:
                    ...
                    st.dataframe(double_buy, hide_index=True, use_container_width=True)
                else:
                ...
                with c_f:
                    ...
                    st.dataframe(inv_df.sort_values('외국인', ascending=False).head(10)[['종목명', '등락률', '외국인']], hide_index=True)
                with c_i:
                    st.subheader("🐯 기관 순매수 Top 10")
                    st.dataframe(inv_df.sort_values('기관합계', ascending=False).head(10)[['종목명', '등락률', '기관합계']], hide_index=True)
            else:
